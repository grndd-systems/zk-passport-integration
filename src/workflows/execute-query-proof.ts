 ...
 * 2. Derives session key and passport hash from registration proof (or uses provided ones)
 * 3. Calls QueryProofExecutor.execute() with the proof
 * 4. Verifies the transaction and displays the result
 *
 * Prerequisites:
...
export async function executeQueryProofWorkflow(params: ExecuteQueryProofParams) {
  ...
  // Step 2: Derive or use provided session key
  const sessionKey =
    params.sessionKey ||
    (() => {
